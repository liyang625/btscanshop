...
Page({  
  ...
  onShareAppMessage: function () {
    ...
    onLoad:function(options){  
        ...
        var vipcode = appUserInfo.vipcode;
    ...
    GetCode: function (e) {
        ...
        var appUserInfo = app.getAppUserInfo();
        if (that.data.phone.indexOf("*") != -1) {
          var phone = appUserInfo.phone;
        } else {
          var phone = that.data.phone;
        }
        wx.request({
            url: app.getUrl("sendPhoneCode"),
            data: app.getData({ phone: phone, type:"bind" }),
    ...
    formSubmit: function (e) {
        ...
        var appUserInfo = app.getAppUserInfo();
        if (e.detail.value.idcard.indexOf("*") != -1) {
          var idcard = appUserInfo.idcard;
        } else {
          var idcard = e.detail.value.idcard;
        }
        if (e.detail.value.phone.indexOf("*") != -1) {
          var phone = appUserInfo.phone;
        } else {
          var phone = e.detail.value.phone;
            ...
            wx.request({
                url: app.getUrl("bind"),
                data: app.getData({ username: e.detail.value.name, cardcode: e.detail.value.vipcode, idcode: idcard, code: e.detail.value.code, phone: phone}),
                header: app.getHeader(),
                success: function(res) {
                	if(wx.hideLoading){
                		wx.hideLoading();
                    ...
                    if(res.data.Error==0){
                      wx.showToast({ title: '绑定成功', duration:2000,image: '../../images/error.png' });
                      wx.navigateBack();
                      // wx.reLaunch({ url: '/pages/a_grhy/a_grhy' });
                    }else{
  ...
  checkSubmit:function(e){
    ...
    if (!myreg.test(phone)){ //验证未通过
        wx.showToast({title: "请输入正确的手机号"});
        return false; 
    }
    if (app.Trim(e.detail.value.code).length == 0) {
